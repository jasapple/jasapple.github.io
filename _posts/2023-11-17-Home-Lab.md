...
# Inspiration
I initially started with a decommissioned SuperMicro server from my university, RIT, which contained a TYAN S7002-LE motherboard. After lugging the server around for a few years, I decided to utilize the hardware in an effective manor along with several other pieces older hardware I had around.

# Homelab 1.0
## Rack
My first task was to setup a central place for all my hardware. As I move around for work a decent amount, I purchased an enclosed server rack with wheels. I came across the [18U NavePoint Rack](https://navepoint.com/navepoint-18u-600mm-depth-networking-cabinet-performance-series/){:target="_blank"} on FaceBook Market. The listing also provided a [4U server chassis](https://www.amazon.com/dp/B0816YQN6X){:target="_blank"} which can support an EATX motherboard, fitting the TYAN S7002-LE motherboard. 

## Primary Server
The TYAN S7002-LE came with two Xeon 5645 processors that I decided to keep using. I first attempted to use the power supply from the original server but it did not have enough proper power connecters to power the board and an old Radeon 5770 I had. I ended up getting a Corsair RM750x to handle the load.

For heatsinks, I got two [be quiet! Shadow Rock LP BK002](https://www.newegg.com/be-quiet-shadow-rock-lp-bk002/p/13C-001F-00006?Item=9SIA68V2TR2312){:target="_blank"} and the original ones were passively cooled from the original chassis loud fans.

SSDs used were two kingston 256GB with the intention of keeping them in a software RAID 1.

I ordered two 8GB DDR3 DIMMS for 16GB of memory.

For the underlying hypervisor, I choose [ProxMox 7.4](http://proxmox.com){:target="_blank"}. I ran a few VMs to test using Proxmox but I did not launch anything for a specific purpose.
...
In my apartment I had Comcast (ew) as an ISP where I used a [Motorola MB8600](https://www.motorola.com/us/mb8600/p){:target="_blank"} as it supports DOCSIS 3.1 and I did not want to manage/deal with the standard Comcast modem.

## Power
[APC 1500VA Back-UPS Pro](https://www.apc.com/us/en/product/BR1500G/apc-backups-pro-1500va-865w-tower-120v-10x-nema-515r-outlets-avr-lcd-user-replaceable-battery/){:target="_blank"}
...
## Laptop Setup
My primary Laptop is a 2013 15" MacBook Pro (retina) which I often use in 'clamshell' mode hooked up to a 32" [LG display}(https://www.lg.com/us/monitors/lg-32MA68HY-P-led-monitor){:target="_blank"}. I would run this through a Belkin Thunderbolt Dock which offers a wired network connection, an additional Thunderbolt out which I used for display out, and a USB hub. I had my laptop on a pull out [rack shelf](https://www.amazon.com/gp/product/B07N1WH4CQ){:target="_blank"} for convince and had a longer thunderbolt cable and power cable ziptied to the shelf. The video out went to a HDMI switch which fed the 32" LG display I used.


# Homelab 1.1

I got tired of reaching into the rack to get to my networking gear, such as reseating connections (stupid Comcast), or reconnecting USB devices. I decided to add a customizable patch panel into the mix.
...
## Hue Bridge
I have a Philips Hue Bridge and prefer using HomeKit to control the lights. As I was reworking the rack and setting up the pi-hole, I figured I try and power the bridge via PoE. I used a TP-Link [TL-PoE10R](https://www.amazon.com/gp/product/B003CFATQK){:target="_blank"} PoE splitter along with a [2.1mm to 2.5mm](https://www.amazon.com/gp/product/B01N812NKL){:target="_blank"} barrel plug adapter. This allowed the Hue Bridge to be powered by PoE and reduce the need for another wall-wart power supply in the rack.


# Homelab 1.2
## NAS
I had several harddrives with my data in several places. I wanted to consolidate. Also, I ran out of space in my TimeMachine Drive and had not backed up my laptop in over a year, so it was needed. I opted for a [Synology DS220+](https://www.synology.com/en-us/products/DS220+){:target="_blank"} and a single [WD Red Pro WD181KFGX 18TB](https://www.westerndigital.com/products/internal-drives/wd-red-pro-sata-hdd?sku=WD181KFGX){:target="_blank"} as there was a great Amazon Prime day deal (Via Newegg) on these. I got a second WD Red Pro WD181KFGX 18TB on a black Friday deal to keep the drives in a RAID 1 for data resiliency.


# HomeLab 2.0
At this point, I moved apartments and learned from some shortfalls in my design. One major shortfall was that the handles on the Primary Server chassis prevented the rack from being closed. I normally had the front panel off so this was not an everyday issue. I had to remove the server from the rack in order for the rest of my equipment to be moved safely. During the moving process, I pulled the drives from the NAS and carried those with me in anti-static bags.
